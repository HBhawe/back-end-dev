...
app.get("/jokes/:id", (req, res) => {
  ...
  const joke = jokes.find((element) => element.id === id);
  res.json(joke);
});
...
app.post("/jokes", (req, res) => {
  const jokeType = String(req.body.type);
  const jokeText = String(req.body.text);
...
app.put("/jokes/:id", (req, res) => {
  ...
  const updatedJoke = {
    id,
    jokeType: String(req.body.type),
    jokeText: String(req.body.text),
  };
...
app.delete("/jokes/:id", (req, res) => {
  ...
  // returns -1 if no match found, 1 for match found
  if (searchIndex > 0) {
    jokes.splice(searchIndex, 1);
    res.sendStatus(200);
  } else {
    res.status(404).send({ error: `Joke with id: ${id} not found.` });
  }
});
...
app.delete("/all", (req, res) => {
  ...
  if (masterKey === userKey) {
    ...
    res
      .status(404)
      .send({ error: `You are not authorised to perform this action.` });
  }
});
